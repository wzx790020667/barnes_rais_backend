...
export class EngineModelRuleService {
  ...
  async searchEngineModelRules(page: number, pageSize: number, query: string): Promise<{ engineModelRules: EngineModelRule[]; total: number }> {
    return db
      .query(async () => {
        ...
        const { data: countData, error: countError } = await supabase
          .from("engine_model_rules")
          .select("count", { count: "exact" })
          .or(`engine_model_title.ilike.%${query}%,common_prefix.ilike.%${query}%,result_display.ilike.%${query}%`);
