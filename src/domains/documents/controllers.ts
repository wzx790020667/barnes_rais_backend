...
export class DocumentController {
  ...
  async getFilteredDocuments(req: BunRequest): Promise<Response> {
    try {
      ...
      const filter =
        (url.searchParams.get("filter") as "paired" | "unpaired") || "unpaired";
      ...
      if (!pageParam && !pageSizeParam) {
        ...
        const documents = await this.documentService.getAllFilteredDocuments(
          filter
        );
    ...
    try {
      ...
      const document: Omit<Document, "created_at"> = {
        ...
        receive_date: validatedData.receive_date
          ? moment(validatedData.receive_date).toDate()
          : null,
        ...
        t_import_number_page:
          validatedData.t_import_number_page !== undefined
            ? validatedData.t_import_number_page
            : null,
        t_po_number_page:
          validatedData.t_po_number_page !== undefined
            ? validatedData.t_po_number_page
            : null,
        t_end_user_customer_name_page:
          validatedData.t_end_user_customer_name_page !== undefined
            ? validatedData.t_end_user_customer_name_page
            : null,
        t_end_user_customer_number_page:
          validatedData.t_end_user_customer_number_page !== undefined
            ? validatedData.t_end_user_customer_number_page
            : null,
        t_work_scope_page:
          validatedData.t_work_scope_page !== undefined
            ? validatedData.t_work_scope_page
            : null,
        t_arc_requirement_page:
          validatedData.t_arc_requirement_page !== undefined
            ? validatedData.t_arc_requirement_page
            : null,
        t_tsn_page:
          validatedData.t_tsn_page !== undefined
            ? validatedData.t_tsn_page
            : null,
        t_csn_page:
          validatedData.t_csn_page !== undefined
            ? validatedData.t_csn_page
            : null,
        scanned_time: moment().toDate(),
        updated_at: moment().toDate(),
      };

      const engineModelRules =
        await this.engineModelRuleService.getAllEngineModelRules();

      // Prepare document items
      const documentItems =
        validatedData.document_items?.map((item) => ({
          ...
          engine_model: isEmpty(item.engine_model)
            ? null
            : replaceEngineModelTitleByRules(
                engineModelRules,
                String(item.engine_model)
              ),
          engine_number: isEmpty(item.engine_number)
            ? null
            : String(item.engine_number),
          serial_number: isEmpty(item.serial_number)
            ? null
            : String(item.serial_number),
          t_part_number_page:
            item.t_part_number_page !== undefined
              ? item.t_part_number_page
              : null,
          t_quantity_ordered_page:
            item.t_quantity_ordered_page !== undefined
              ? item.t_quantity_ordered_page
              : null,
          t_import_price_page:
            item.t_import_price_page !== undefined
              ? item.t_import_price_page
              : null,
          t_engine_model_page:
            item.t_engine_model_page !== undefined
              ? item.t_engine_model_page
              : null,
          t_engine_number_page:
            item.t_engine_number_page !== undefined
              ? item.t_engine_number_page
              : null,
          t_serial_number_page:
            item.t_serial_number_page !== undefined
              ? item.t_serial_number_page
              : null,
  ...
  async getDocumentFromBucket(req: BunRequest): Promise<Response> {
    try {
      ...
      const document = await this.documentService.getDocumentFromBucket(
        bucketName,
        id
      );
  ...
  async getDocumentsFromBucket(req: BunRequest): Promise<Response> {
    try {
      ...
      const documents = await this.documentService.getDocumentsFromBucket(
        bucketName,
        ids
      );
  ...
  async searchImportNumbers(req: BunRequest): Promise<Response> {
    try {
      ...
      const result = await this.documentService.searchImportNumbers(
        query,
        page,
        pageSize
      );
  ...
  async uploadPdfToExternal(req: BunRequest): Promise<Response> {
    try {
      ...
      return new Response(result?.body, {
        ...
        headers: {
          "Content-Type":
            result?.headers.get("Content-Type") || "application/zip",
          "Content-Disposition":
  ...
  async addImportNumber(req: BunRequest): Promise<Response> {
    try {
      ...
      if (!result.success) {
        return Response.json(
          {
            error: "Invalid import number data",
      ...
      const updatedDocument =
        await this.documentService.updateDocumentImportNumber(
          id,
          import_number
        );
  ...
  async pdfFullARD(req: BunRequest): Promise<Response> {
    try {
      ...
      const task = await this.aiInferenceService.getTrainingTaskByDatasetId(
        customer.t_model_name || ""
      );
      if (!task) {
        return Response.json(
      ...
      const result = await this.aiInferenceService.loadInferenceModel(
        task.t_dataset_id
      );
